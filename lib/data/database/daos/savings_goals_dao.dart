...
class SavingsGoalsDao extends DatabaseAccessor<AppDatabase> with _$SavingsGoalsDaoMixin {
  ...
  Future<SavingsGoal?> getSavingsGoalById(String id) {
    return (select(savingsGoals)..where((t) => t.id.equals(id))).getSingleOrNull();
  }

  Future<int> insertSavingsGoal(SavingsGoal goal) => into(savingsGoals).insert(goal);

  Future<bool> updateSavingsGoal(SavingsGoal goal) => update(savingsGoals).replace(goal);

  Future<void> deleteSavingsGoal(String id) async {
    await transaction(() async {
      // Delete all contributions linked to this goal
      await (delete(savingsGoalContributions)..where((t) => t.savingsGoalId.equals(id))).go();
      // Delete the goal itself
      await (delete(savingsGoals)..where((t) => t.id.equals(id))).go();
    });
  }

  Stream<List<SavingsGoalContribution>> watchContributionsForGoal(String savingsGoalId) {
  ...
  Future<List<SavingsGoalContribution>> getContributionsForGoal(String savingsGoalId) {
    return (select(savingsGoalContributions)
          ..where((t) => t.savingsGoalId.equals(savingsGoalId))
          ..orderBy([(t) => OrderingTerm(expression: t.createdAt, mode: OrderingMode.desc)]))
        .get();
