...
class SmsClassifier {
  ...
  static SmsClassification classify(String rawBody) {
    ...
    if (promoHits.score > 0) {
      ...
      reasons.add('Promo language (${promoHits.matches.join(", ")}) → −${promoHits.score.toStringAsFixed(1)}');
    }
    ...
    if (exclamationCount >= 2) {
      score -= 1.5;
      reasons.add('Multiple exclamation marks ($exclamationCount) → promo urgency');
    }
    ...
    final alphaCount = text.runes.where((r) => (r >= 65 && r <= 90) || (r >= 97 && r <= 122)).length;
    if (alphaCount > 20) {
      final capsRatio = upperCount / alphaCount;
      if (capsRatio > 0.60) {
        score -= 1.0;
        reasons.add('High caps ratio (${(capsRatio * 100).toStringAsFixed(0)}%) → promo style');
      }
    }
    ...
    if (txnVerbHits.score > 0) {
      ...
      reasons.add('Transaction verbs (${txnVerbHits.matches.join(", ")}) → +${txnVerbHits.score.toStringAsFixed(1)}');
    }
    ...
    if (text.length < 200 && text.length > 30) {
      score += 0.5;
      reasons.add('Short structured message (${text.length} chars) → receipt-like');
    }
  ...
  static bool _hasReferencePattern(String text) {
    ...
    if (RegExp(r'(?:Rej|Ref|TxnID|TxnId|Transaction|Kumbukumbu|ID)[:\s]+[A-Za-z0-9]{4,}', caseSensitive: false)
        .hasMatch(text)) {
      return true;
    }
  ...
  static bool _hasBalancePattern(String lower) {
    return RegExp(r'(?:salio|balance|new balance|bal)[:\s]', caseSensitive: false).hasMatch(lower);
  }

  static bool _hasDateTimeNearAmount(String text) {
    // Matches patterns like "on 15/5/2026 at 1:19 PM" or "tarehe 15/5/2026"
    return RegExp(r'(?:on|tarehe)\s+\d{1,2}/\d{1,2}/\d{2,4}', caseSensitive: false).hasMatch(text);
  }

  static bool _hasCurrencyAmount(String lower) {
    return RegExp(r'(?:tsh|tzs|tsh|tshs)\s*[\d,]+', caseSensitive: false).hasMatch(lower);
  }
