...
final dynamicInsightsProvider = FutureProvider<List<InsightData>>((ref) async {
  ...
  final thisMonthTxns = await transactionDao.watchFilteredTransactions(
    startDate: startOfMonth,
    endDate: now,
  ).first;

  final lastMonthTxns = await transactionDao.watchFilteredTransactions(
  ...
  final thisExpenses =
      thisMonthTxns.where((r) => r.transaction.type == 'expense').toList();
  final lastExpenses =
      lastMonthTxns.where((r) => r.transaction.type == 'expense').toList();
  ...
  final thisTotal =
      thisExpenses.fold<int>(0, (s, t) => s + t.transaction.amount);
  final lastTotal =
      lastExpenses.fold<int>(0, (s, t) => s + t.transaction.amount);
  if (lastTotal > 0) {
  ...
  if (catThis.isNotEmpty) {
    final top = catThis.entries.reduce(
      (a, b) => a.value > b.value ? a : b,
    );
    final pct = thisTotal > 0 ? (top.value * 100 ~/ thisTotal) : 0;
    insights.add(InsightData(
      title: top.key,
      subtitle: '${CurrencyFormatter.formatCents(top.value)} \u2014 $pct% of spending',
