...
void main() {
  ...
  late RecurringTransactionRepository recurringTransactionRepo;
  ...
  group('Adaptive Dynamic Auto-Categorization Tests', () {
    test(
      'Learns from past matching transactions and assigns custom categories with high confidence',
      () async {
        final categories = await categoryDao.getAllCategories();
        final rentCategory = categories.firstWhere((c) => c.name == 'Rent');
        final supermarketCategory = categories.firstWhere(
  ...
  group('Automated Transfer & Deduplication Tests', () {
    test(
      'Intercepts P2P transfer as a Transfer transaction and deduplicates counterparty SMS',
      () async {
        ...
        final senderSms =
            'Umetuma Tsh 100,000.00 kwa $receiverPhone tarehe 15/5/2026 saa 14:30. Rej: MPESA01. Salio: Tsh 400,000.00';

        final firstSuccess = await smsProcessor.processSms(
          'M-Pesa_TZ',
          senderSms,
          transferTime,
        );
        ...
        final receiverSms =
            'Umepokea Tsh 100,000.00 kutoka kwa $senderPhone tarehe 15/5/2026 saa 14:30. Rej: TIGO02. Salio: Tsh 300,000.00';

        final secondSuccess = await smsProcessor.processSms(
          'TigoPesa_TZ',
          receiverSms,
          transferTime.add(const Duration(seconds: 30)),
        );
