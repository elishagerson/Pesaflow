...
void main() {
  ...
  group('Drift Local Database & DAOs Verification', () {
    ...
    test('Account additions and balance adjustments work together', () async {
      ...
      final salaryCat = categoriesList.firstWhere(
        (cat) => cat.name == 'Salary',
      );
      final foodCat = categoriesList.firstWhere(
        (cat) => cat.name == 'Food & Groceries',
      );
    ...
    test(
      'approveReviewedTransaction updates source and links with active tracker ID if null',
      () async {
        final uuid = const Uuid();
        final accountId = uuid.v4();
        ...
        var retrieved = await (database.select(
        ...
        retrieved = await (database.select(
          database.transactions,
        )..where((t) => t.id.equals(transaction.id))).getSingle();
    ...
    test(
      'tracker deletion cascade deletes transactions, savings goals and contribution adjustments',
      () async {
        final uuid = const Uuid();
        final accountId = uuid.v4();
        ...
        await database
            .into(database.savingsGoalContributions)
            .insert(contribution);

        // Verify they exist in DB
        final goalsBefore = await (database.select(
          database.savingsGoals,
        )..where((t) => t.trackerId.equals(trackerId))).get();
        expect(goalsBefore.length, 1);
        final txsBefore = await (database.select(
          database.transactions,
        )..where((t) => t.trackerId.equals(trackerId))).get();
        ...
        final trackersAfter = await (database.select(
          database.trackers,
        )..where((t) => t.id.equals(trackerId))).get();
        expect(trackersAfter.isEmpty, true);

        final txsAfter = await (database.select(
          database.transactions,
        )..where((t) => t.trackerId.equals(trackerId))).get();
        expect(txsAfter.isEmpty, true);

        final goalsAfter = await (database.select(
          database.savingsGoals,
        )..where((t) => t.trackerId.equals(trackerId))).get();
        expect(goalsAfter.isEmpty, true);

        final contributionsAfter = await (database.select(
          database.savingsGoalContributions,
        )..where((t) => t.savingsGoalId.equals(goalId))).get();
