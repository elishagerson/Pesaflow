#!/bin/bash
# Replaces the HEADER block entirely in transaction_list_screen.dart

# First find the exact start and end line of the // ── HEADER ── block
START_LINE=$(grep -n "// ── HEADER ──" lib/presentation/transactions/transaction_list_screen.dart | cut -d: -f1)
# Find the next block after HEADER which is // ── TRANSACTION LIST ──
END_LINE=$(grep -n "// ── TRANSACTION LIST ──" lib/presentation/transactions/transaction_list_screen.dart | cut -d: -f1)

# We want to replace everything between START_LINE and END_LINE-1
cat << 'INNER_EOF' > header_replacement.dart
            // ── HEADER ──
            Positioned(
              top: 0,
              left: 0,
...
sed -i "${START_LINE},$(($END_LINE - 1))c\\$(cat header_replacement.dart | sed 's/$/\\/g' | sed '$s/\\$//')" lib/presentation/transactions/transaction_list_screen.dart
